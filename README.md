...
- 🦊 **AI キャラクター常駐** → マスコットが吹き出しでタスク状況にコメント。完了したら褒めてくれて、期限切れには発破をかけてくる。ミニキャラのお祝い演出付き。キャラ名・口調は `config/character.json` でカスタマイズ可能
...
`name`(キャラ名)・`emoji`(ミニキャラ演出に使用)・`persona`(口調・性格の説明文)を編集すると、アプリ内の AI キャラクターが変わります。デフォルトはフランクな相棒キャラ「やしろAI」です。
